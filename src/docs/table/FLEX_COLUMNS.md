# Flexible Columns (AG-Grid Style)
...
// → Flexible column (minmax(100px, 1fr))
// → NOT resizable
```

### Programmatic Resize Reset
...
const resetColumnWidths = () => {
  // All resized columns return to their original state
  tableRef.value?.resetWidths();
};
</script>
```
...
```typescript
const columns = [
  { key: "name" },   // Default: flexible
  { key: "email" },  // Default: flexible
];
```

### Changes

1. `width: "2fr"` → remove (default flexible)
2. `width: "auto"` → remove (default flexible)
3. `width: "150px"` → `width: "150px"` (no changes)
